...
def force_kill_chrome_processes():
    """Ferme de force tous les processus Chrome orphelins sur le VPS"""
    try:
        # Commande Linux pour tuer tous les processus liés à chrome
        os.system("pkill -f chrome")
        print("🧹 Nettoyage forcé du système effectué avec succès.")
...
def safe_get(url, max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        if not is_driver_alive(driver):
            print("⚠️ Driver mort détecté avant chargement, nettoyage et recréation...")
            try:
                driver.quit()
            except Exception:
                pass
            force_kill_chrome_processes()  # On s'assure que rien ne reste en RAM
            driver = create_driver()
        ...
        try:
            ...
            if "receiving message from renderer" in msg:
                print("Renderer bloqué -> Nettoyage forcé et recréation immédiate du driver")
                try:
                    driver.quit()
                except Exception:
                    pass
                force_kill_chrome_processes()  # Nettoyage système
                driver = create_driver()
            else:
                try:
            ...
            print(f"Erreur driver critique sur {url} (tentative {attempt+1}) : {e}")
            try:
                driver.quit()
            except Exception:
                pass
...
for index, row in df.iterrows():
    ...
    if processed:
        ...
        companies_scraped += 1
        if companies_scraped % 10 == 0:
            print("♻️ Recyclage préventif global du driver pour purger la RAM du VPS...")
            try:
                driver.quit()
            except Exception:
                pass
